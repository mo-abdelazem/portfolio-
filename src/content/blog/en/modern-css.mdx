...
[Specificity wars](/blog/css-selectors-specificity-cascade) — where you bump a selector or add `!important` just to win — get solved structurally by **cascade layers**. You declare an explicit order, and later layers beat earlier ones regardless of specificity:
...
Together they let you derive a whole palette (hovers, tints, shades, borders) from a single base color — the kind of thing that used to require Sass color functions. (Full treatment, including gradients, dark mode, and contrast, in [CSS color & gradients](/blog/css-color-gradients).)
...
Building on regular [transitions and keyframe animations](/blog/css-transitions-animations), animations can now be tied to scroll position **without JavaScript**, running on the compositor for smoothness:
