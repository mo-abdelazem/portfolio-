...
Now `getPosts()` is cached and revalidated like a tagged fetch — `revalidateTag("posts")` clears it too. (Next.js is moving toward a `'use cache'` directive for the same purpose; the concept is identical — mark a function's result as cacheable and tag it.)
...
The newest piece ties it together. **Partial Prerendering** lets a single route be *mostly static with dynamic holes*: Next.js prerenders a static shell and streams the dynamic parts (wrapped in `<Suspense>`) into it at request time. You get the instant first paint of a static page **and** per-request data on the same route, without choosing one or the other for the whole page. It's opt-in (still stabilizing), but it's the direction the model is heading: static by default, dynamic exactly where you wrap it.
