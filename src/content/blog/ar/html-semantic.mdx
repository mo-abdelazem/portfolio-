...
### تمرين 1 — أصلِح حساء الـ div

أعد كتابة هذا بعناصر دلالية:
...
كل landmark (`<header>`، `<nav>`، `<main>`، `<footer>`) و`<article>` يخبر المتصفح وقارئات الشاشة بما *هي* المنطقة — شيء لا يبلّغه `<div class="menu">` أبدًا.

</details>

### تمرين 2 — ترتيب العناوين

عنوان بطاقة يبدو صغيرًا بصريًّا، فيعلّمه مطوّر `<h4>` "لأنه الحجم الصحيح". لماذا هذا خطأ، وماذا ينبغي أن يفعل؟

<details>

<summary>إظهار الحل</summary>

العناوين تنقل **البنية، لا الحجم** — القفز من `<h1>` إلى `<h4>` يكسر مخطّط المستند الذي تعتمد عليه قارئات الشاشة وSEO. استخدم المستوى الذي يطابق التسلسل (`<h2>` لقسم فرعيّ مباشر)، ثم اجعله *صغيرًا* بـ CSS (`font-size`)، لا باختيار وسم عنوان أعمق.

</details>

### تمرين 3 — زر أم رابط؟

عنصر تحكّم يشغّل دالّة JavaScript لحذف صفّ — لا ينتقل إلى أي مكان. أينبغي أن يكون `<a>` أم `<button>`، ولماذا؟

<details>

<summary>إظهار الحل</summary>

**`<button>`**. الروابط تنتقل إلى عنوان URL؛ والأزرار تؤدّي فعلًا. الـ `<button>` قابل للتركيز، ويُطلَق على Enter وSpace معًا، ويُعلَن كزرّ للتقنيات المساعِدة — لا شيء من ذلك يصيبه `<a href="#">` بمعالِج نقر. القاعدة: ينتقل ← رابط؛ يفعل شيئًا ← زر.

</details>

### تمرين 4 — سمِّ حقلًا

اجعل هذا الإدخال وصوليًّا:
...
العنصر النائب ليس تسمية — يختفي عند الإدخال وتتخطّاه كثير من قارئات الشاشة. أما `<label>` حقيقية مربوطة عبر `for`/`id` فتسمّي الحقل وتجعل مساحته الأكبر تركّز الإدخال.

</details>

## النموذج الذهني الذي تحتفظ به

HTML الدلالي يعني اختيار العناصر بناءً على ما *هو* المحتوى، لا كيف يبدو. ابنِ الصفحة من **landmarks** (`<header>`، `<nav>`، `<main>`، `<footer>`، `<article>`، `<aside>`) كي تُعلَن مناطقها ويمكن التنقّل فيها؛ وأبقِ **العناوين مرتّبة** (`<h1>` ← `<h2>` ← …) لأنها مخطّط المستند، لا أحجام خطوط؛ واستخدم **`<button>` للأفعال و`<a>` للتنقّل**؛ وأعطِ كل إدخال **`<label>`** حقيقية. ولا تلجأ إلى `<div>`/`<span>` مجرّد إلا حين لا يناسب عنصر ذو معنى. افعل ذلك وتأتي الوصولية وSEO ودعم لوحة المفاتيح شبه مجّانًا — المتصفح يفهم أصلًا ما تعنيه عناصرك. ومع بنية ذات معنى، تنسّقها [CSS](/blog/css-box-model) ويجعلها [JavaScript](/blog/javascript-fundamentals) تفاعلية.
